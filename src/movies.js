...
function orderByYear(moviesArray) {
    let moviesArrayClone = moviesArray.slice();
    moviesArrayClone.sort((a, b) => {
        if (a.year === b.year){
            if (a.title > b.title) {
                return 1;
...
function orderAlphabetically(moviesArray) {
    let top20Movies = moviesArray.slice();
    top20Movies.sort((a, b) => {
        if (a.title > b.title) {
            return 1;
        } else if (a.title == b.title) {
            return 0;
    ...
    top20Movies;      
        return top20Movies.map((element) => element.title).slice(0, 20);    
}
